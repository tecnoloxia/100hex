...
if (hexagono==true){
    union(){
        ...
        intersection(){
            cylinder(r=60, h=altura, center=false, $fn=6);
            linear_extrude(altura) scale(12.5) rotate(0) translate([-l/2.15,-l/4,0]) dragon(nivel);  
        }
    }  
}
else {
    linear_extrude(altura) scale(12) rotate(0) translate([-l/2.4,-l/4,0]) dragon(nivel);   
}  
